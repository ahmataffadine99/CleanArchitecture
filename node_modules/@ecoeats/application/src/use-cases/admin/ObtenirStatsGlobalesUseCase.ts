...
export class ObtenirStatsGlobalesUseCase {
  ...
  async executer() {
    const [commandes, comptes, restaurants] = await Promise.all([
    ...
    const commandesAujourdhui = commandes.filter(c => c.getCreeLe() >= hier).length;
    const commandesHier = commandes.filter(c => c.getCreeLe() < hier && c.getCreeLe() >= new Date(hier.getTime() - 24*3600*1000)).length;
    ...
    const nouveauxComptesHier = comptes.filter(c => {
      return c.creeLe && c.creeLe >= hier;
    }).length;

    const nouveauxRestaurantsSemaine = restaurants.filter(r => {
    ...
    const commandesEnAttente = commandes.filter(c => c.getStatut() === "EN_ATTENTE").length;
