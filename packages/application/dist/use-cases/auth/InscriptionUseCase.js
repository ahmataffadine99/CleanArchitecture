...
class InscriptionUseCase {
    ...
    async executer(req) {
        ...
        if (req.role === "CLIENT") {
            ...
            const restaurant = new domain_1.Restaurant(profilId, req.nom,
            req.adresse || "Adresse à préciser", new domain_1.Coordonnees(req.latitude || 48.8566, req.longitude || 2.3522), profilId
            );
            await this.depotRestaurants.sauvegarder(restaurant);
        }
        else if (req.role === "LIVREUR") {
            const livreur = new domain_1.Livreur(profilId, req.nom, new domain_1.Coordonnees(req.latitude || 48.8566, req.longitude || 2.3522), req.telephone || "À renseigner", false,
            domain_1.Money.zero()
            );
