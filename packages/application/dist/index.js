...
var require_bcrypt = __commonJS({
  "../../node_modules/bcrypt/bcrypt.js"(exports2, module2) {
    ...
    function genSalt(rounds, minor, cb) {
      ...
      crypto3.randomBytes(16, function(error2, randomBytes) {
        if (error2) {
          cb(error2);
          return;
        }
...
__export(index_exports, {
  ...
  AccepterLivraisonUseCase: () => AccepterLivraisonUseCase,
...
var InscriptionUseCase = class {
  ...
  async executer(req) {
    ...
    if (existant) throw new import_domain2.EmailDejaUtiliseError(req.email);
    const motDePasseHache = await import_bcrypt.default.hash(req.motDePasse, this.SALT_ROUNDS);
    const profilId = v4_default();
    if (req.role === "CLIENT") {
      const client = new import_domain.Client(profilId, req.nom, req.email, req.adresse || "\xC0 renseigner", req.telephone);
    ...
    const compteId = v4_default();
    const compte = new import_domain.CompteUtilisateur(
      compteId,
      req.email,
      motDePasseHache,
      req.role,
...
var ConnexionUseCase = class {
  ...
  async executer(req) {
    ...
    if (!compte) throw new import_domain3.IdentifiantsInvalidesError();
    const motDePasseValide = await import_bcrypt2.default.compare(req.motDePasse, compte.motDePasseHache);
    if (!motDePasseValide) throw new import_domain3.IdentifiantsInvalidesError();
...
var AjouterAuPanierUseCase = class {
  ...
  async executer(req) {
    ...
    if (!plat.estDisponible()) {
      throw new import_domain5.PlatEnRuptureError(plat.id);
    }
    const panier = this.obtenirOuCreerPanier(req.clientId);
    const article = new import_domain4.ArticlePanier(
      plat.id,
      plat.nom,
      plat.prix,
      req.quantite,
  ...
  getTousLesPaniersParRestaurant(restaurantId) {
    return Array.from(this.paniers.values()).filter(
      (p) => !p.estVide() && p.getRestaurantId() === restaurantId
    );
  }
  getPanier(clientId) {
  ...
  obtenirOuCreerPanier(clientId) {
    if (!this.paniers.has(clientId)) {
      this.paniers.set(clientId, new import_domain4.Panier(clientId));
    }
...
var PasserCommandeUseCase = class {
  ...
  calculPrix = new import_domain7.CalculPrixService();
  async executer(req) {
    ...
    const commande = new import_domain6.Commande(
      v4_default(),
      req.clientId,
      restaurantId,
      [...req.panier.getArticles()],
...
var PayerCommandeUseCase = class {
  ...
  async executer(req) {
    ...
    commande.changerStatut(import_domain8.StatutCommande.PAYEE);
    const facture = new import_domain8.Facture(
      v4_default(),
      commande.id,
      req.clientId,
    ...
    if (this.depotClients) {
      try {
        ...
        console.warn(`[Fid\xE9lit\xE9] Client ${req.clientId} introuvable pour cr\xE9diter les points.`);
      }
    }
    ...
    return Promise.all(commandes.map(async (cmd) => {
      try {
        ...
        return {
          ...
          clientTelephone: "Non renseign\xE9",
          statut: cmd.getStatut(),
          creeLe: cmd.getCreeLe(),
          prixPlatsCentimes: cmd.getPrixPlats().enCentimes(),
          id: cmd.id,
...
var ModifierRestaurantUseCase = class {
  ...
  async executer(req) {
    ...
    if (req.nom !== void 0) restaurant.nom = req.nom;
...
var AttribuerLivraisonUseCase = class {
  ...
  selectionLivreur = new import_domain16.SelectionLivreurService();
  async executer(req) {
    const commande = await this.depotCommandes.trouverParId(req.commandeId);
    if (commande.getStatut() !== import_domain15.StatutCommande.PRETE) {
...
var TerminerLivraisonUseCase = class {
  ...
  calculGains = new import_domain17.CalculGainsLivreurService();
  async executer(req) {
    const commande = await this.depotCommandes.trouverParId(req.commandeId);
    const livreur = await this.depotLivreurs.trouverParId(req.livreurId);
    const restaurant = await this.depotRestaurants.trouverParId(commande.restaurantId);
