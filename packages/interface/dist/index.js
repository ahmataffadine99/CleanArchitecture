...
__export(index_exports, {
  ...
  creerRoutesAuth: () => creerRoutesAuth,
...
function creerRoutesClient(deps) {
  ...
  router.get("/favoris/details/:clientId", async (req, res, next) => {
    try {
  ...
  router.get("/restaurant/mon-restaurant", async (req, res, next) => {
    try {
      if (!req.user || !req.user.profilId) {
        return res.status(401).json({ error: "Non connect\xE9" });
      }
      const restaurant = await deps.obtenirMonResto.executer(req.user.profilId);
      if (!restaurant) {
        return res.status(404).json({ error: "Aucun restaurant trouv\xE9 pour ce propri\xE9taire" });
      }
      res.json(restaurant);
    } catch (err) {
      next(err);
...
function creerAuthMiddleware(secretJwt) {
  return function authMiddleware(req, res, next) {
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      return res.status(401).json({ error: "Token JWT manquant ou mal format\xE9" });
    }
    const token = authHeader.split(" ")[1];
    try {
      const decoded = import_jsonwebtoken.default.verify(token, secretJwt);
      req.user = {
        sub: decoded.sub,
        role: decoded.role,
      ...
      return res.status(401).json({ error: "Token JWT invalide ou expir\xE9" });
    }
  };
}
function requireRole(role) {
  return (req, res, next) => {
    if (!req.user || req.user.role !== role) {
      return res.status(403).json({ error: `Acc\xE8s refus\xE9. R\xF4le ${role} requis.` });
    }
    next();
  };
}
