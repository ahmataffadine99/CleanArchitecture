  ...
  if (step === 'success') {
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center animate-in fade-in slide-in-from-bottom duration-700">
        ...
        <div className="bg-white border border-slate-100 rounded-[3rem] p-10 max-w-md mx-auto text-left shadow-2xl shadow-slate-200/50">
          <h3 className="font-black text-slate-800 mb-6 border-b border-slate-100 pb-4 text-xl tracking-tight">Résumé de Facturation</h3>
          <ul className="space-y-4 mb-8">
            <li className="flex justify-between items-center text-sm">
               <span className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Facture ID</span>
               <span className="font-mono text-xs bg-slate-50 px-3 py-1 rounded-lg text-slate-600 border border-slate-100">{facture?.factureId}</span>
            </li>
            <li className="text-sm text-slate-600 leading-relaxed bg-emerald-50/50 p-6 rounded-2xl border border-emerald-50 italic font-medium">
               "{facture?.detail}"
            </li>
          </ul>
          ...
          <button 
            onClick={() => navigate('/')}
            className="w-full bg-slate-900 text-white font-black py-5 rounded-2xl hover:bg-black transition-all shadow-xl shadow-slate-200"
          >
            RETOUR À L'ACCUEIL
          </button>
        </div>
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
      <div className="flex items-center gap-6 mb-12">
        <button 
          onClick={() => navigate(-1)}
          className="p-4 bg-white rounded-2xl shadow-xl shadow-slate-100 border border-slate-50 text-slate-400 hover:text-emerald-500 transition-all hover:-translate-x-1"
        >
          <ChevronLeft size={24} />
        </button>
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">Finalisez votre commande</h1>
          <p className="text-slate-500 font-medium mt-1">Étape finale avant la dégustation.</p>
        </div>
      </div>

      {error && (
        <div className="bg-red-50 text-red-600 p-6 rounded-[2rem] border border-red-100 mb-10 font-bold shadow-lg shadow-red-100 text-center animate-in shake duration-500">
          {error}
        </div>
      )}

      {items.length === 0 && step !== 'processing' ? (
        <div className="bg-white p-20 rounded-[3rem] border border-slate-100 border-dashed text-center shadow-xl shadow-slate-100/50">
          <p className="text-2xl font-black text-slate-300 mb-6">Votre panier semble vide...</p>
          <button onClick={() => navigate('/')} className="bg-emerald-500 text-white font-black px-10 py-5 rounded-2xl shadow-xl shadow-emerald-100 hover:bg-emerald-600 transition-all">DÉCOUVRIR LES RESTAURANTS</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-3 space-y-10">
            <form onSubmit={processOrder} className="bg-white p-10 lg:p-12 rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/40 space-y-12">
              <section>
                <div className="flex items-center gap-4 mb-10">
                  <div className="p-3 bg-emerald-100 text-emerald-600 rounded-2xl">
                    <MapPin size={24} />
                  </div>
                  <h2 className="text-2xl font-black text-slate-800 tracking-tight">Destination</h2>
                </div>
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white rounded-2xl px-6 py-4 outline-none transition-all font-bold text-slate-700 placeholder:text-slate-300"
